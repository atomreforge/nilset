...
- 主题由 `ThemeRepository` 提供 `StateFlow<UserThemeSettings>`，启动时从 DataStore 恢复；默认使用枫糖，另含落樱、青碧、汀蓝、动态取色和自定义主题。卡片与导航容器使用明暗适配的透明度遮罩，主题配色主要影响图标与文字。裁剪后的全局背景图按应用比例生成，透明度默认 100%。
