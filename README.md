...
- 课表共建页当前用户的课表以本地 DataStore 为准，创建、编辑和删除先写本地；断开服务端时可离线修改，恢复连接后会对比本地与远端核心字段并以本地数据覆盖远端。支持创建和编辑课程（标题、星期、分段时间、老师、课室和备注）、长按课程删除、问候、下一节课提示、星期筛选、课程列表和下拉刷新。查看他人课表时隐藏课程编辑入口并禁用长按操作。成员列表目前仍只显示当前登录用户；服务端已提供公共单用户课表读取，多人聚合和成员列表仍待后续 API。
