...
- 登录与注册 API 接入；注册支持服务端字段校验、确认密码和注册码输入，成功后返回登录页并预填用户名。
- 设置页顶部提供用户卡片，显示昵称和用户名；长按可退出登录并返回登录页。头像当前仅保留 `avatar` 空字段。
...
- 课表共建页当前用户的课表以本地 DataStore 为准，创建、编辑和删除先写本地；断开服务端时可离线修改，恢复连接后会对比本地与远端核心字段并以本地数据覆盖远端。支持创建和编辑课程（标题、星期、分段时间、老师、课室和备注）、长按课程删除、问候、下一节课提示、星期筛选、课程列表和下拉刷新。查看他人课表时隐藏课程编辑入口并禁用长按操作。成员列表目前为临时占位，只显示当前登录用户，多人共享和他人课表读取待服务端提供 API。
...
- 主题页支持浏览并选择 TTF/OTF 字体文件作为全局字体；文件会复制到应用私有目录并校验，卡片下方显示当前字体，可随时恢复默认字体。
