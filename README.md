...
- 主题模式提供浅色和深色；默认使用枫糖，另含落樱、青碧、汀蓝、动态取色和自定义，内置配色均含浅色与深色版本，四色自定义结果通过 DataStore 恢复。
- 动态取色主题在 Android 12+ 按当前浅色/深色模式跟随系统配色。
- 主题页支持选择本地图片并在应用内按应用比例裁剪后设为全局背景；背景图默认 100% 透明度，可调整为 0%-100%。卡片、顶栏和底部导航使用基于当前背景色派生的透明度遮罩；侧边栏使用不透明底，并在自定义背景下对齐全局背景位置。主题配色主要影响图标与文字。文本缩放和 UI 缩放可通过 80%-120% 滑块调整，设置均通过 DataStore 恢复。
